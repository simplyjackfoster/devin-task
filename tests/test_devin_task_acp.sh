...
decide 'cat x 2>&1 | head' read && ok "read allows 2>&1 before a pipe" || fail "2>&1 pipe denied"
decide 'cat x >&2' read && ok "read allows >&2 (dup with no leading descriptor)" || fail ">&2 denied"
decide 'cat x >&-' read && ok "read allows >&- (closing a descriptor)" || fail ">&- denied"
! decide 'cat x >& f' read && ok "read denies >& word (a file redirect, not a dup)" || fail ">& word allowed"
! decide 'cat x >&/tmp/y' read && ok "read denies >&/path with no space" || fail ">&/path allowed"
! decide 'cat x >&1a' read && ok "read denies >&1a (not a bare descriptor)" || fail ">&1a allowed"
...
! decide "sed -ni.bak s/a/b/ f" read && ok "read denies sed -ni.bak (bundled cluster)" || fail "sed -ni.bak allowed"
! decide "sed -ni p f" read && ok "read denies sed -ni (bundled cluster)" || fail "sed -ni allowed"
! decide "sed -I.bak s/a/b/ f" read && ok "read denies sed -I.bak (BSD in-place)" || fail "sed -I.bak allowed"
